...
| [tests/](../../tests/) | Unit and synthetic integration coverage (`tests/llm/unit/`, `tests/vlm/unit/`, `tests/integration/`). |
...
└── vlm/
    ├── components/
    │   ├── language/    # Decoder-side pruning stack (Qwen2.5-VL, Qwen3-VL)
    │   ├── vision/      # Vision-component estimators and pruners
    │   └── merger/      # Merger-component estimators and pruners
...
- **Registries.** `ESTIMATOR_REGISTRY`, `PRUNER_REGISTRY`, `PRUNING_STRATEGY_REGISTRY` per domain (`llm.core`, `vlm.components.language.core`, `vlm.components.vision.core`, and `vlm.components.merger.core`). Factories `create_estimator()` / `create_pruner()` route by method-first / effect-first keys.
...
Canonical pruning pipeline (LLM and VLM language / vision / merger components):
...
- Component-scoped VLM registries: `language`, `vision`, and `merger` expose separate estimator / pruner namespaces while preserving shared manifest conventions.
