...
class TeacherCorrection:
    ...
    def train(self):
        ...
        for epoch in range(num_epochs):
                    ...
                    if self.accelerator.sync_gradients:
                        ...
                        if completed_steps % log_every_n_steps == 0:
                            avg_loss = self.accelerator.gather(
                                loss.repeat(self.config.get('per_device_train_batch_size', 4))
                            ).mean()
            ...
            avg_epoch_loss = self.accelerator.gather(total_loss).sum().item() / (
                len(self.train_loader)
                * self.accelerator.num_processes
                * self.config.get('per_device_train_batch_size', 4)
            )
