...
from ._utils import clone_or_share
...
def build_layerwise_plan(model, spec: ArchitectureSpec, config: LayerWiseConfig) -> PruningPlan:
    num_layers = len(spec.get_layers(model))
...
def apply_blockwise_plan(
    ...
    for layer_metadata in context.layer_metadata:
        handles = spec.get_layer_handles(pruned_model, layer_metadata.layer_idx)
...
def apply_channelwise_plan(
    ...
    tied_embeddings = _is_tied_embedding(model, spec)

    source_embed = spec.get_embed_tokens(pruned_model)
    ...
    pruned_model.model.embed_tokens = new_embed

    final_norm = spec.get_final_norm(pruned_model)
    pruned_model.model.norm = _slice_norm(final_norm, residual_indices)

    lm_head = spec.get_lm_head(pruned_model)
    if lm_head is not None and hasattr(lm_head, "weight"):
        new_lm_head = _slice_linear(lm_head, out_indices=None, in_indices=residual_indices)
        if tied_embeddings:
            new_lm_head.weight = pruned_model.model.embed_tokens.weight
        pruned_model.lm_head = new_lm_head

    for layer_idx in range(len(spec.get_layers(pruned_model))):
        handles = spec.get_layer_handles(pruned_model, layer_idx)
        layer = handles.layer
        ...
        attention_module = handles.attention.module
        attention_module.q_proj = _slice_linear(
            handles.attention.q_proj,
            out_indices=residual_indices,
            in_indices=residual_indices,
        )
        attention_module.k_proj = _slice_linear(
            handles.attention.k_proj,
            out_indices=kv_indices,
            in_indices=residual_indices,
        )
        attention_module.v_proj = _slice_linear(
            handles.attention.v_proj,
            out_indices=kv_indices,
            in_indices=residual_indices,
        )
        attention_module.o_proj = _slice_linear(
            handles.attention.o_proj,
            out_indices=residual_indices,
            in_indices=residual_indices,
        )
        spec.patch_attention_metadata(
            attention_module,
        ...
        mlp_module = handles.mlp.module
        mlp_module.gate_proj = _slice_linear(handles.mlp.gate_proj, out_indices=None, in_indices=residual_indices)
        mlp_module.up_proj = _slice_linear(handles.mlp.up_proj, out_indices=None, in_indices=residual_indices)
        mlp_module.down_proj = _slice_linear(handles.mlp.down_proj, out_indices=residual_indices, in_indices=None)

    spec.patch_model_hidden_size(pruned_model, new_hidden_size)
...
def _is_tied_embedding(model, spec: ArchitectureSpec) -> bool:
    embed = spec.get_embed_tokens(model)
    lm_head = spec.get_lm_head(model)
