...
class HybridDistiller:
    ...
    def _resolve_teacher_kept_layers(self, block_layers_to_prune):
        total_layers = text_num_hidden_layers(self.teacher_model)
        ...
        student_layers = text_num_hidden_layers(self.student_model)
        if student_layers != len(teacher_kept):
            raise ValueError(
                f"Student has {student_layers} layers, expected {len(teacher_kept)}."
            )
        return teacher_kept
    ...
    def _ensure_distill_model(self, device_student, block_layers_to_prune):
        ...
        if rebuild:
            student_dim = text_hidden_size(self.student_model)
            teacher_dim = text_hidden_size(self.teacher_model)
    ...
    def _forward_teacher(self, batch, device_teacher):
        ...
        teacher_batch["output_hidden_states"] = True
        ...
        *,
        ...
        with torch.no_grad():
            for batch in val_loader:
                prepared_batch, labels, loss_mask = prepare_causal_lm_batch(batch, device_student)
                teacher_outputs = self._forward_teacher(prepared_batch, device_teacher)
                student_outputs = self._forward_student(distill_model, prepared_batch)
