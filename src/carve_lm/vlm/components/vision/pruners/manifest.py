...
from ...language.adapters import import_object, object_path
from ._utils import config_from_dict, instantiate_model, json_dump, json_load
from .types import DiscoveryContext, PruningPlan, PruningResult

MANIFEST_FILENAME = "vlm_vision_pruner_manifest.json"
...
def load_pruned_result(pruner_cls, output_dir, device: str | None = None, dtype=None) -> PruningResult:
    ...
    base_config = config_from_dict(config_cls, manifest["base_config"])
    model = instantiate_model(model_cls, base_config, device=device, dtype=dtype)

    pruner_config_cls = import_object(manifest["config_class"])
    config = config_from_dict(pruner_config_cls, manifest["config_payload"])
